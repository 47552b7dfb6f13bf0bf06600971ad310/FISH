...
export default defineEventHandler(async (event) => {
  try {
    ...
    const ticket = await DB.Ticket.findOne({ code: code, user: auth._id }).select('cancel complete code spot') as IDBTicket
    if(!ticket) throw 'Vé này không còn tồn tại'
    if(!!ticket.cancel) throw 'Vé này đã bị hủy'
    if(!!ticket.complete.pay.pending) throw 'Không thể hủy vé câu đã thanh toán'
