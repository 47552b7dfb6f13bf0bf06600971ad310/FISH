import type { IAuth, IDBItem, IDBTicket, IDBLakeArea, IDBLakeSpot, IDBConfig, IDBUser } from "~~/types"
import md5 from "md5"
...
export default defineEventHandler(async (event) => {
  try {
    ...
    const ticket = await DB.Ticket.findOne({ code: code }).select('user code cancel status area spot') as IDBTicket
    ...
    // Get User
    const user = await DB.User.findOne({ _id: ticket.user }).select('member') as IDBUser
    if(!user) throw 'Không tìm thấy dữ liệu khách hàng'
    ...
    // Check Waiting
    const has = await DB.TicketOrder.count({ ticket: ticket._id, user: user._id, status: 0 }) 
    ...
    return resp(event, { message: 'Đặt dịch vụ thành công' })
  } 
  catch (e:any) {
