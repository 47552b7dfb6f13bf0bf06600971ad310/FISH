...
const action = async () => {
  try {
    changing.value = true
    await useAPI('ticket/public/spot/change', { _id: spot.value._id, ticket: props.ticket._id })
...
const getSpots = async () => {
  try {
    ...
    loading.value = true
    const data = await useAPI('lake/public/spot/empty', { area: props.ticket.area._id })
