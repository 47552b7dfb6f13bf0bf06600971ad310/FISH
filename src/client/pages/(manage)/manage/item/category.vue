<template>
  <UiContent title="Loại Sản Phẩm" sub="Quản lý các loại sản phẩm bán thêm">
    <UiFlex class="gap-1">
    ...
    <UModal v-model="modal.add" preventClose>
      <UForm :state="stateAdd" @submit="addAction" class="bg-card rounded-2xl p-4">
        <UFormGroup label="Loại sản phẩm">
...
const getList = async () => {
  try {
    ...
    const data = await useAPI('item/manage/category/list', JSON.parse(JSON.stringify(page.value)))
...
const addAction = async () => {
  try {
    ...
    await useAPI('item/manage/category/add', JSON.parse(JSON.stringify(stateAdd.value)))
...
const editAction = async () => {
  try {
    ...
    await useAPI('item/manage/category/edit', JSON.parse(JSON.stringify(stateEdit.value)))
...
const delAction = async (_id) => {
  try {
    ...
    await useAPI('item/manage/category/del', { _id })
